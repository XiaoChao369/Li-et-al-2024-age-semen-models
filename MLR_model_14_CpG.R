...
# 
# Output: the donor's age estimated by the MLR_model_14_CpG model.
...
setwd(dirModel)
MLR_model_14_CpG <- readRDS("MLR_model_14_CpG.RDS")

## Prepare data ---------------------------------------------------------------
# Read the methylation data
setwd(dirData)
meth_data <- data.table::fread("SN001_MLR_model_14_CpG.txt", header = TRUE)
...
# 14:       SN001   chr1.19339492   52.74330
...
setwd(dirResult)
write.csv(meth_data, "meth_data_MLR_model_14_CpG.csv", quote = FALSE, row.names = FALSE)

# Convert long data to wide data
meth_data <- ungroup(meth_data) %>%
  tidyr::spread(key = Marker, value = Meth_level)

#  Sample.Name chr1.19339492 chr1.19339503 chr10.4384066 chr11.89589683 chr12.123465627 chr12.4535197
#        SN001       52.7433      59.85325      45.66817       62.36365        51.33506      73.74337
...
## Estimate age using the MLR_model_14_CpG model ---------------------------------------
# Estimate age
estimated_age <- data.frame(
  Sample.Name = meth_data$Sample.Name,
  Estimated.Age = predict(MLR_model_14_CpG, newdata = meth_data[,-1])
)
# Save estimated ages into a .csv file
setwd(dirResult)
write.csv(estimated_age, "estimated_age_MLR_model_14_CpG.csv", quote = FALSE, 
          row.names = FALSE)
